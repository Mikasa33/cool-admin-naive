...
const title = computed(() => `${unref(isEdit) ? '编辑' : '新建'}商品`)
...
async function handleOpen(params: { edit?: boolean; id?: number | string }) {
  ...
  if (params?.edit) {
    try {
      const data = await product.info({ id: params.id })
...
async function handleConfirm() {
  try {
    ...
    toggleInit(true)
    toggleLoading(true)
    await product[unref(isEdit) ? 'update' : 'add'](unref(formRef).getFieldsValue())
