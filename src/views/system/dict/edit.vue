...
const title = computed(() => `${unref(isEdit) ? '编辑' : '新建'}字典`)
const [init, toggleInit] = useToggle()
const [loading, toggleLoading] = useToggle()

async function handleOpen(params: { edit?: boolean; id?: number | string; typeId: number; parentId?: number | string }) {
  await open()
  toggleInit(true)
  isEdit.value = params?.edit || false
  unref(formRef).init(schemas({ typeId: params.typeId }))
  if (params?.edit) {
    try {
