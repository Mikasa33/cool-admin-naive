...
const title = computed(() => `${unref(isEdit) ? '编辑' : '新建'}文章`)
...
async function handleOpen(params: { edit?: boolean; id?: number | string }) {
  ...
  if (params?.edit) {
    try {
      const data = await article.info({ id: params.id })
...
async function handleConfirm() {
  try {
    ...
    toggleInit(true)
    toggleLoading(true)
    await article[unref(isEdit) ? 'update' : 'add'](unref(formRef).getFieldsValue())
